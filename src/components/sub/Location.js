...
const Location = () => {
  ...
  //클릭 된 경우 전달할 좌표의 값 모음
  // 서버에서 좌표값을 가지고 와서 처리
  const infoArr = [
    {
      title: "스타벅스 동성로광장점",
      latLng: new kakao.maps.LatLng(35.867294, 128.5942), //지도의 중심좌표.
  ...
  useEffect(() => {
    ...
    //btns는 ref객체
    //btns의 current 는 ul
    //btns.current.children은 하위요소 즉 li
    //이렇게하면 총 3개의 목록이 접근
    ...
    //btns의 current.children 중 idx에 해당하는
    //요소만 on 클래스 적용
  ...
  return (
    <Layout title={"Location"}>
      ...
      <div className="btnSet">
        <ul ref={btns}>
          {
            //jsx에 js코드하려면 {사이에 코딩한다.}
            //목록이 여러개의 데이터가 모인 배열이다.
            //jsx에서 그렇다면 무조건, map을 사용하자
            //또는 jsx에서 여러번 동일하게 반복해야 한다면
            //map을 쓰자.
            //item: 각각의 값
            //index: 순서번호
            // info.map()
